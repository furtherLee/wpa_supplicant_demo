					   ...
					   struct p2p_group_member *m)
{
	...
	if (coupled_sink) {
		wpabuf_put_u8(buf, coupled_sink[0]);
		/* FIX: Where to get MAC address of coupled peer sink? */
		wpabuf_put_data(buf, zero_addr, ETH_ALEN);
	} else {
		wpabuf_put_u8(buf, 0);
		wpabuf_put_data(buf, zero_addr, ETH_ALEN);
...
int p2p_group_match(struct p2p_group *g, const u8 *group_id,
		    size_t group_id_len)
{
	if (group_id_len < ETH_ALEN)
		return 0;

	if (os_memcmp(g->p2p->cfg->dev_addr, group_id, ETH_ALEN) != 0)
		return 0; /* Mismatch in P2P Device Address?! */

	return group_id_len == ETH_ALEN ||
		(g->cfg->ssid_len == group_id_len - ETH_ALEN &&
		 os_memcmp(g->cfg->ssid, group_id + ETH_ALEN, g->cfg->ssid_len)
		 == 0);
}
