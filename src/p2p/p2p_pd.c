					       ...
					       struct p2p_device *go)
{
	...
	if (p2p->wfd_ie_prov_disc)
		extra = wpabuf_len(p2p->wfd_ie_prov_disc);
...
#ifdef CONFIG_WIFI_DISPLAY
	if (p2p->wfd_ie_prov_disc)
		wpabuf_put_buf(buf, p2p->wfd_ie_prov_disc);
#endif /* CONFIG_WIFI_DISPLAY */
...
#ifdef CONFIG_WIFI_DISPLAY
	struct wpabuf *wfd_ie = p2p->wfd_ie_prov_disc;
	if (wfd_ie && group_id) {
		size_t i;
		for (i = 0; i < p2p->num_groups; i++) {
			struct p2p_group *g = p2p->groups[i];
			struct wpabuf *ie;
			if (!p2p_group_match(g, group_id, group_id_len))
				continue;
