...
#ifdef CONFIG_HS20
        os_free(conf->hs20_venue_name);
        os_free(conf->hs20_network_auth_type);
        os_free(conf->hs20_nai_realm_list);
        os_free(conf->hs20_3gpp_cellular_network);
        os_free(conf->hs20_domain_name_list_value);
        os_free(conf->hs20_operator_friendly_name);
