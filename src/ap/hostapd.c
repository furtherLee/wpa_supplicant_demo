...
#ifdef CONFIG_HS20
	wpabuf_free(hapd->hs20_beacon_ie);
	hapd->hs20_beacon_ie = NULL;
	wpabuf_free(hapd->hs20_probe_resp_ie);
	hapd->hs20_probe_resp_ie = NULL;
	hs20_ap_deinit(hapd);
#endif
}
...
#if CONFIG_HS20
	if (hs20_ap_init(hapd)) {
		wpa_printf(MSG_ERROR, "HS20 initialization failed.");
		return -1;
	}
#endif  /* CONFIG_HS20 */
...
/* FIX: must not call directly into hostapd/main.c */
void hostapd_iface_free(struct hostapd_iface *iface);
int hostapd_iface_driver_init(struct hostapd_iface *iface);
int hostapd_setup_iface(struct hostapd_iface *iface);
...
int hostapd_enable_iface(struct hostapd_iface *hapd_iface)
{
	...
	if (hostapd_iface_driver_init(hapd_iface) ||
	    hostapd_setup_iface(hapd_iface)) {
		hostapd_iface_free(hapd_iface);
		return -1;
	}
	return 0;
}
...
/* FIX: Must not call directly into hostapd/ctrl_iface.c */
...
hostapd_iface_alloc(struct hapd_interfaces *interfaces)
{
	...
	if (interfaces->count == 0) {
		interfaces->iface = os_zalloc(sizeof(struct hostapd_iface *));
...
hostapd_config_alloc(struct hapd_interfaces *interfaces, char *buf)
{
	struct hostapd_bss_config *bss;
	struct hostapd_config *conf;
	...
	ptr = os_strchr(buf, ' ');
	*ptr = '\0';
	os_strlcpy(vapname, buf, sizeof(vapname));
	...
	/* After ..defaults() iface[0] is '\0': Hence assign vapname*/
	os_strlcpy(bss->iface, (char *)vapname, sizeof(bss->iface));
	bss->ctrl_interface = os_strdup(ptr + 1);
...
int hostapd_add_iface(struct hapd_interfaces *interfaces, char *buf)
{
	...
	char vapname[IFNAMSIZ], buffer[60], *ptr;
	size_t i, vap_count;

	os_strlcpy(buffer, buf, sizeof(buffer));
	ptr = os_strchr(buf, ' ');
	*ptr = '\0';
	os_strlcpy(vapname, buf, sizeof(vapname));
	...
	for (i = 0; i < interfaces->count; i++) {
		if (!os_strcmp(interfaces->iface[i]->conf->bss[0].iface,
			       vapname)) {
			wpa_printf(MSG_ERROR, "VAP already exists!!");
			return -1;
		}
	}
	vap_count = i;

	if (vap_count == interfaces->count) {
		...
		conf = hostapd_config_alloc(interfaces, buffer);
		if (conf == NULL) {
			wpa_printf(MSG_ERROR, "%s: Failed to allocate memory "
		...
		if (hapd_iface->ctrl_iface_init &&
			hapd_iface->ctrl_iface_init(hapd_iface->bss[0])) {
			wpa_printf(MSG_ERROR, "%s: Failed to setup control "
				   "interface", __func__);
			goto fail;
		}
		wpa_printf(MSG_ERROR, "%s: %s", __func__, conf->bss[0].iface);
...
int hostapd_remove_iface(struct hapd_interfaces *interfaces, char *buf)
{
	...
	for (i = 0; i < interfaces->count; i++) {
		...
		if (hapd_iface == NULL)
			return -1;
		if (!os_strcmp(hapd_iface->conf->bss[0].iface, vapname)) {
			hostapd_iface_free(hapd_iface);
			k = i;
	...
	wpa_printf(MSG_ERROR, "%s: %s", __func__, buf);
	return 0;
}
