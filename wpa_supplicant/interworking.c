					    ...
					    const u8 *data, size_t slen)
{
	...
	case ANQP_VENUE_NAME:
		...
		wpa_hexdump_ascii(MSG_INFO, "ANQP: Venue Name", pos, slen);
		if (bss) {
	...
	case ANQP_NETWORK_AUTH_TYPE:
		wpa_msg(wpa_s, MSG_INFO, "RX-ANQP " MACSTR
			...
			MAC2STR(sa));
		wpa_hexdump_ascii(MSG_INFO, "ANQP: Network Authentication "
				  "Type", pos, slen);
		if (bss) {
	...
	case ANQP_ROAMING_CONSORTIUM:
		...
		wpa_hexdump_ascii(MSG_INFO, "ANQP: Roaming Consortium",
				  pos, slen);
		if (bss) {
	...
	case ANQP_IP_ADDR_TYPE_AVAILABILITY:
		wpa_msg(wpa_s, MSG_INFO, "RX-ANQP " MACSTR
			...
			MAC2STR(sa));
		wpa_hexdump(MSG_INFO, "ANQP: IP Address Availability",
			    pos, slen);
		if (bss) {
	...
	case ANQP_NAI_REALM:
		...
		wpa_hexdump_ascii(MSG_INFO, "ANQP: NAI Realm", pos, slen);
		if (bss) {
	...
	case ANQP_3GPP_CELLULAR_NETWORK:
		...
		wpa_hexdump_ascii(MSG_INFO, "ANQP: 3GPP Cellular Network",
				  pos, slen);
		if (bss) {
	...
	case ANQP_DOMAIN_NAME:
		...
		wpa_hexdump_ascii(MSG_INFO, "ANQP: Domain Name", pos, slen);
		if (bss) {
