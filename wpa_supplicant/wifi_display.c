...
static int wifi_display_update_wfd_ie(struct wpa_global *global)
{
	...
	if (!global->wifi_display) {
		...
		p2p_set_wfd_ie_prov_disc(global->p2p, NULL);
	 ...
	 *
	 * Beacon, Probe Request, (Re)Association Request, GO Negotiation
	 * Req/Resp/Conf:
	 * WFD Device Info
	 * [Associated BSSID]
	 * [Coupled Sink Info]
	 *
	 * Probe Response, (Re)Association Response, P2P Invitation Req/Resp,
	 * Provision Discovery Req/Resp:
	 * same as above and also [WFD Session Info]
	 */
	len = 0;
	...
	ie = wifi_display_encaps(buf);
	wpa_hexdump_buf(MSG_DEBUG, "WFD: WFD IE for Provision Discovery", ie);
	p2p_set_wfd_ie_prov_disc(global->p2p, ie);
