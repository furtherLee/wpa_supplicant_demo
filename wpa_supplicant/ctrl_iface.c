					 ...
					 char *cmd, char *buf, size_t buflen)
{
	...
	wpa_printf(MSG_INFO, "CTRL_IFACE GET '%s'", cmd);
	...
	if (os_strcmp(cmd, "version") == 0) {
		...
		return res;
                ...
                wpa_printf(MSG_INFO, "gtk_len = %d", wpa_s->last_gtk_len);
	...
	struct wpa_supplicant *wpa_s, char *addr)
{
	...
	if (hwaddr_aton(addr, peer)) {
		wpa_printf(MSG_INFO, "CTRL_IFACE TDLS_DISCOVER: invalid "
			   "address '%s'", addr);
		return -1;
	}

	wpa_printf(MSG_INFO, "CTRL_IFACE TDLS_DISCOVER " MACSTR,
		   MAC2STR(peer));

	if (wpa_tdls_is_external_setup(wpa_s->wpa)) {
	        printf("TDLS_DISCOVER: call wpa_tdls_send_discovery_request\n");
		ret = wpa_tdls_send_discovery_request(wpa_s->wpa, peer);
	} else {
               printf("TDLS_DISCOVER: wpa_drv_tdls_oper\n");
					...
					char *dst)
{
	...
	if (len == 0 && wpa_s->conf->home_realm)
		return hs20_nai_home_realm_list(wpa_s, dst_addr,
						wpa_s->conf->home_realm);
